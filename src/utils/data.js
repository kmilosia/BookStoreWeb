export const productSortOptions = [
    {value: '', label: 'Domyślne'},
    {value: '&sortBy=alphabetical&sortOrder=desc', label: 'Alfabetycznie - rosnąco'},
    {value: '&sortBy=alphabetical&sortOrder=asc', label: 'Alfabetycznie - malejąco'},
    {value: '&sortBy=price&sortOrder=desc', label: 'Cena - malejąco'},
    {value: '&sortBy=price&sortOrder=asc', label: 'Cena - rosnąco'},
]
export const searchSortOptions = [
    {value: 'default', label: 'Domyślne'},
    {value: 'az', label: 'Od A do Z'},
    {value: 'za', label: 'Od Z do A'},
]
export const sortOptions = [
    {value: 'default', label: 'Domyślne'},
    {value: 'az', label: 'Od A do Z'},
    {value: 'za', label: 'Od Z do A'},
    {value: 'cheap', label: 'Cena rosnąco'},
    {value: 'expensive', label: 'Cena malejąco'},
]

export const categoryData = [
    { title: "Science Fiction", logo: 'https://iili.io/JBf7eUb.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.', src: 'https://iili.io/JCJgR9e.png' },
    { title: "Poezja", logo: 'https://iili.io/JBf7EJf.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.',  src: 'https://iili.io/JCJgTc7.png'  },
    { title: "Fantasy", logo: 'https://iili.io/JBf70UG.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.',  src: 'https://iili.io/JCJguS9.png'  },
    { title: "Kulinarne", logo: 'https://iili.io/JBf7YiX.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.',  src: 'https://iili.io/JCJgxP2.png'  },
    { title: "Komiks", logo: 'https://iili.io/JBf77Vt.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.',  src: 'https://iili.io/JCJgK9n.png'  },
    { title: "Biografia", logo: 'https://iili.io/JBf7ADN.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.',  src: 'https://iili.io/JCJgnn4.png'  },
    { title: "Komedia", logo: 'https://iili.io/JBf7TRR.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.',  src: 'https://iili.io/JCJgxP2.png'  },
    { title: "Dla dzieci", logo: 'https://iili.io/JBf7MOl.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.',  src: 'https://iili.io/JCJgK9n.png'  },
    { title: "Lifestyle", logo: 'https://iili.io/JBf7Vb2.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.',  src: 'https://iili.io/JCJg2at.png'  },
    { title: "Podręcznik", logo: 'https://iili.io/JBf7js9.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.',  src: 'https://iili.io/JCJgqwG.png'  },
    { title: "Romans", logo: 'https://iili.io/JBf7Nfe.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.',  src: 'https://iili.io/JCJgBtf.png'  },
    { title: "Nauka", logo: 'https://iili.io/JBf7O0u.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.',  src: 'https://iili.io/JCJgnn4.png'  },
    { title: "Dla młodzieży", logo: 'https://iili.io/JBf7vJj.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.',  src: 'https://iili.io/JCJgoMl.png'  },
    { title: "Horror", logo: 'https://iili.io/JBf7GR4.png', content: 'Eksploruj nowe galaktyki z nieziemską fantastyką i SciFi. Wejdź do świata literatury i słuchaj bądź czytaj ponad 400 000 audiobooków i ebooków - zawsze i gdziekolwiek masz ochotę.',  src: 'https://iili.io/JCJgIFS.png'  },
];

export const booksData = [
    {id: 1, title: "Harry Potter i Więzień Azkabanu",price: 59.99, score: 5, author: 'J.K. Rowling', url: 'https://images.ctfassets.net/usf1vwtuqyxm/24YWmI4UcyoMwj7wdKrEcL/374de1941927db12bd844fb197eab11f/English_Harry_Potter_3_Epub_9781781100233.jpg?w=914&q=70&fm=jpg'},
    {id: 2, title: "Wiedźmin. Sezon Burz",price: 39.99, score: 4, author: 'Andrzej Sapkowski', url: 'https://smakliter.pl/Photos/10/LARGE/000025/wiedzmin-sezon-burz.jpg'},
    {id: 3, title: "Ogień i Krew. Gra o Tron. Część 1",price: 59.99, score: 4, author: 'George R.R. Martin', url: 'https://image.ceneostatic.pl/data/article_picture/f9/8e/fca2-c9c4-4c76-94f5-0799189bdcff_large.jpg'},
    {id: 4, title: "Harry Potter i Więzień Azkabanu",price: 59.99, score: 5, author: 'J.K. Rowling', url: 'https://images.ctfassets.net/usf1vwtuqyxm/24YWmI4UcyoMwj7wdKrEcL/374de1941927db12bd844fb197eab11f/English_Harry_Potter_3_Epub_9781781100233.jpg?w=914&q=70&fm=jpg'},
    {id: 5, title: "Wiedźmin. Sezon Burz",price: 39.99, score: 4, author: 'Andrzej Sapkowski', url: 'https://smakliter.pl/Photos/10/LARGE/000025/wiedzmin-sezon-burz.jpg'},
    {id: 6, title: "Ogień i Krew. Gra o Tron. Część 1",price: 59.99, score: 4, author: 'George R.R. Martin', url: 'https://image.ceneostatic.pl/data/article_picture/f9/8e/fca2-c9c4-4c76-94f5-0799189bdcff_large.jpg'},
]

export const bannersData = [
    {id: 1,path: '/', title: 'Powitanie', src: 'https://iili.io/Jxo3RLB.png'},
    {id: 2,path: '/promocje', title: 'Świąteczne promocje', src: 'https://iili.io/Jxo37qP.png'},
    {id: 3,path: '/', title: 'Newsletter', src: 'https://iili.io/Jxo3Imx.png'},
    {id: 4,path: '/promocje', title: 'Black Friday', src: 'https://iili.io/Jxo3uzQ.png'},
    {id: 5,path: '/ebooki', title: 'Ebooki', src: 'https://iili.io/Jxo3AXV.png'},
]

export const productsData = [
    {id: 1, form: 'ebook', file: "EPUB", title: "Harry Potter i Więzień Azkabanu",price: 59.99, score: 5, author: 'J.K. Rowling', url: 'https://images.ctfassets.net/usf1vwtuqyxm/24YWmI4UcyoMwj7wdKrEcL/374de1941927db12bd844fb197eab11f/English_Harry_Potter_3_Epub_9781781100233.jpg?w=914&q=70&fm=jpg'},
    {id: 2, form: 'ebook', file: "MOBI", title: "Wiedźmin. Sezon Burz",price: 39.99, score: 4, author: 'Andrzej Sapkowski', url: 'https://smakliter.pl/Photos/10/LARGE/000025/wiedzmin-sezon-burz.jpg'},
    {id: 3, form: 'ebook', file: "PDF", title: "Ogień i Krew. Gra o Tron. Część 1",price: 59.99, score: 4, author: 'George R.R. Martin', url: 'https://image.ceneostatic.pl/data/article_picture/f9/8e/fca2-c9c4-4c76-94f5-0799189bdcff_large.jpg'},
    {id: 4, form: 'ebook', file: "EPUB", title: "Harry Potter i Więzień Azkabanu",price: 59.99, score: 5, author: 'J.K. Rowling', url: 'https://images.ctfassets.net/usf1vwtuqyxm/24YWmI4UcyoMwj7wdKrEcL/374de1941927db12bd844fb197eab11f/English_Harry_Potter_3_Epub_9781781100233.jpg?w=914&q=70&fm=jpg'},
    {id: 5, form: 'ebook', file: "PDF", title: "Wiedźmin. Sezon Burz",price: 39.99, score: 4, author: 'Andrzej Sapkowski', url: 'https://smakliter.pl/Photos/10/LARGE/000025/wiedzmin-sezon-burz.jpg'},
    {id: 6, form: 'ebook', file: "PDF", title: "Ogień i Krew. Gra o Tron. Część 1",price: 59.99, score: 4, author: 'George R.R. Martin', url: 'https://image.ceneostatic.pl/data/article_picture/f9/8e/fca2-c9c4-4c76-94f5-0799189bdcff_large.jpg'},
    {id: 7, form: 'book', edition: "Okładka miękka", title: "Harry Potter i Więzień Azkabanu",price: 59.99, score: 5, author: 'J.K. Rowling', url: 'https://images.ctfassets.net/usf1vwtuqyxm/24YWmI4UcyoMwj7wdKrEcL/374de1941927db12bd844fb197eab11f/English_Harry_Potter_3_Epub_9781781100233.jpg?w=914&q=70&fm=jpg'},
    {id: 8, form: 'book', edition: "Okładka twarda", title: "Wiedźmin. Sezon Burz",price: 39.99, score: 4, author: 'Andrzej Sapkowski', url: 'https://smakliter.pl/Photos/10/LARGE/000025/wiedzmin-sezon-burz.jpg'},
    {id: 9, form: 'book', edition: "Okładka miękka", title: "Ogień i Krew. Gra o Tron. Część 1",price: 59.99, score: 4, author: 'George R.R. Martin', url: 'https://image.ceneostatic.pl/data/article_picture/f9/8e/fca2-c9c4-4c76-94f5-0799189bdcff_large.jpg'},
    {id: 10, form: 'book', edition: "Okładka miękka", title: "Harry Potter i Więzień Azkabanu",price: 59.99, score: 5, author: 'J.K. Rowling', url: 'https://images.ctfassets.net/usf1vwtuqyxm/24YWmI4UcyoMwj7wdKrEcL/374de1941927db12bd844fb197eab11f/English_Harry_Potter_3_Epub_9781781100233.jpg?w=914&q=70&fm=jpg'},
    {id: 11, form: 'book', edition: "Okładka twarda", title: "Wiedźmin. Sezon Burz",price: 39.99, score: 4, author: 'Andrzej Sapkowski', url: 'https://smakliter.pl/Photos/10/LARGE/000025/wiedzmin-sezon-burz.jpg'},
    {id: 12, form: 'book', edition: "Okładka twarda", title: "Ogień i Krew. Gra o Tron. Część 1",price: 59.99, score: 4, author: 'George R.R. Martin', url: 'https://image.ceneostatic.pl/data/article_picture/f9/8e/fca2-c9c4-4c76-94f5-0799189bdcff_large.jpg'},
]

export const infiniteCarouselData = [
    {id: 1, form: 'ebook', title: 'The Great Gatsby', src: 'https://thatcovergirl.files.wordpress.com/2011/04/fitzgerald_tgg.jpg'},
    {id: 2, form: 'ebook', title: 'Krew Elfów', src: 'https://www.orbitbooks.net/wp-content/uploads/2022/06/Sapkowski_BloodofElves-HC-scaled.jpg'},
    {id: 3, form: 'ebook', title: 'The Summer of Broken Rules', src: 'https://images-na.ssl-images-amazon.com/images/S/compressed.photo.goodreads.com/books/1600698670i/49259788.jpg'},
    {id: 4, form: 'ebook', title: 'Ogień i Krew', src: 'https://d-art.ppstatic.pl/kadry/k/r/1/b1/c4/5bb1f0cd9d443_o_large.jpg'},
    {id: 5, form: 'ebook', title: 'New World', src: 'https://www.creativindie.com/wp-content/uploads/2017/04/New-WOrld-299x449.jpg'},
    {id: 6, form: 'ebook', title: 'Czas Pogardy', src: 'https://juandahlmann.files.wordpress.com/2013/09/czaspogardy.jpg'},
    {id: 7, form: 'ebook', title: 'Gra o Tron', src: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS-s0vgI6YMiu9H3XwK8YFAkpsLbbs2Q9CGPw&usqp=CAU'},
    {id: 8, form: 'ebook', title: 'Kamień Filozoficzny', src: 'https://ew.com/thmb/rvat6WP-MplFuFNA4xJ-aGKEADc=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/9781408855652-png-c57764456b554308ae1398474caab3c2.jpg'},
    {id: 9, form: 'ebook', title: 'The Book of Art', src: 'https://d1csarkz8obe9u.cloudfront.net/posterpreviews/art-book-cover-design-template-34323b0f0734dccded21e0e3bebf004c_screen.jpg?ts=1637015198'},
    {id: 10, form: 'ebook', title: 'Spring Book', src: 'https://content.wepik.com/statics/90897927/preview-page0.jpg'},
    {id: 11, form: 'ebook', title: 'The Cursed Child', src: 'https://bukovero.com/wp-content/uploads/2016/07/Harry_Potter_and_the_Cursed_Child_Special_Rehearsal_Edition_Book_Cover.jpg'},
    {id: 12, form: 'ebook', title: 'The Hypocrite World', src: 'https://template.canva.com/EAD7WuSVrt0/1/0/1003w-LVthABb24ik.jpg'},
    {id: 13, form: 'ebook', title: 'Goblet of fire', src: 'https://images.ctfassets.net/usf1vwtuqyxm/3d9kpFpwHyjACq8H3EU6ra/85673f9e660407e5e4481b1825968043/English_Harry_Potter_4_Epub_9781781105672.jpg'},
    {id: 14, form: 'ebook', title: 'The Fate of Fausto', src: 'https://www.creatopy.com/blog/wp-content/uploads/2020/07/The-Fate-of-Fausto.jpg'},
    {id: 15, form: 'ebook', title: 'Moonwarden', src: 'https://d.wattpad.com/story_parts/831849177/images/1611aa04f3b0715a816103127645.jpg'},
    {id: 16, form: 'ebook', title: 'Last Hope', src: 'https://imgv3.fotor.com/images/gallery/Fiction-Book-Covers.jpg'},
    {id: 17, form: 'ebook', title: 'Spare', src: 'https://static01.nyt.com/images/2023/01/12/multimedia/09HARRY-COVER-1-19ca/09HARRY-COVER-1-19ca-articleLarge.jpg?quality=75&auto=webp&disable=upscale'},
    {id: 18, form: 'ebook', title: 'Saga', src: 'https://www.writersandartists.co.uk/sites/default/files/advice/old/admin_1-asset-5afa9df219116.JPG'},
    {id: 19, form: 'ebook', title: 'Thrill of the Chase', src: 'https://fiverr-res.cloudinary.com/image/upload/f_auto,q_auto,t_delivery_large/v1/attachments/delivery/asset/b6bd1c892c4e759efea30f700999d3ab-1667834565/Book%20Cover.png'},
]
